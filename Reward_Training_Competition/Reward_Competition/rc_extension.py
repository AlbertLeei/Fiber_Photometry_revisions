...
class Reward_Competition(RTC):
    ...
    def compute_subject_mean_traces_non_reward(self,
                                    df: pd.DataFrame,
                                    event_type: str) -> pd.DataFrame:
        """
        For each subject in `df`:
        ...
        # group by subject
        for (subj, initiator), subdf in df.groupby(["subject_name", "initiator"]):
            session_means = []
            session_lengths = []
    ...
    def build_subject_traces_df(self, event_label="Competition Bout"):
        ...
        for trial_name, trial in self.trials.items():
            ...
            for initiator in trial.behaviors["Initiators"].unique():
