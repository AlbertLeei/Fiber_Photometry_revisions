"""
Run the shared Home Cage preprocessing pipeline and save cached experiments.

This stops before notebook-specific plotting and summary tables, so the same
.pkl files can be reused across HC analysis notebooks without reloading raw TDT
blocks or recomputing dopamine metrics.
"""
...
DEFAULT_MPFC_EXPERIMENT_PATH = (
    r"C:\Users\alber\OneDrive\Desktop\PC_Lab\Photometry\Pilot_2"
    r"\Combined_Cohorts\Home_Cage\mpfc"
)
DEFAULT_MPFC_CSV_PATH = (
    r"C:\Users\alber\OneDrive\Desktop\PC_Lab\Photometry\Pilot_2"
    r"\Combined_Cohorts\Home_Cage\mpfc_csvs"
)
DEFAULT_NAC_OUTPUT_PATH = Path(__file__).with_name("preprocessed_home_cage_nac.pkl")
DEFAULT_MPFC_OUTPUT_PATH = Path(__file__).with_name("preprocessed_home_cage_mpfc.pkl")


BOUT_DEFINITIONS = [
    {
        "prefix": "Short_Term",
        "introduced": "Short_Term_Introduced",
        "removed": "Short_Term_Removed",
    },
    {
        "prefix": "Long_Term",
        "introduced": "Long_Term_Introduced",
        "removed": "Long_Term_Removed",
    },
    {
        "prefix": "Novel",
        "introduced": "Novel_Introduced",
        "removed": "Novel_Removed",
    },
]
...
def build_preprocessed_experiment(
    ...
    hc_exp = Experiment(experiment_path, csv_path)
    run_default_batch_process_verbose(hc_exp)
    hc_exp.group_extract_manual_annotations(
        bout_definitions=BOUT_DEFINITIONS,
        first_only=first_only,
    )
    hc_exp.compute_all_da_metrics(
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Preprocess Home Cage NAc and mPFC data and save cached experiments."
    )
    parser.add_argument(
        "--region",
    ...
    parser.add_argument(
        "--mpfc-all-events",
        action="store_true",
        help="Keep all annotated events per bout for mPFC instead of first-only.",
    )
...
def main():
    ...
    if args.region in ("all", "nac"):
        print("\n=== Preprocessing Home Cage NAc ===", flush=True)
